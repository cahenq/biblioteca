        ...
        @foreach ($livros as $livro)
            <div class="col-2 mb-4 text-center book" data-letter="{{ strtoupper(substr($livro->titulo, 0, 1)) }}">
                <div class="card" style="border: none; height: 100%;">
                    <img src="{{ $livro->image_path ? asset('storage/' . $livro->image_path) : asset('images/default-image.jpg') }}" 
                    class="card-img-top" 
                    alt="{{ $livro->titulo }}" 
                    style="height: 250px; object-fit: cover;">                
                    <div class="card-body" style="padding: 10px; display: flex; flex-direction: column; justify-content: space-between;">
                        <h5 class="card-title" style="font-size: 14px; font-weight: normal;">{{ $livro->titulo }}</h5>
                        <p class="card-text" style="font-size: 12px;">{{ $livro->autor }}</p>
                        <a href="{{ route('detalhe-livro', $livro->id) }}" class="btn btn-light" style="background-color: #d5d7da; color: #000">Detalhes</a>
                    </div>
                </div>
            </div>
        @endforeach
